...
2. Create a `.env` file in the root directory with the required configuration:
   ...
   **Note:** `RUNESCAPE_URL` is required. The application automatically appends `?table=0&page={pageNumber}` when fetching leaderboard data.
...
The application will be available at `http://localhost:3000`. The database (SQLite) is created automatically on first run.
